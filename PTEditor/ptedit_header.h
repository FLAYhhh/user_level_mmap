...
static inline size_t ptedit_phys_read_pread(size_t address) {
    ...
    DeviceIoControl(ptedit_fd, PTEDITOR_READ_PHYS_VAL, (LPVOID)&address,
                    sizeof(address), (LPVOID)&val, sizeof(val), &returnLength,
                    0);
#endif
    return val;
}
...
static inline void ptedit_phys_write_pwrite(size_t address, size_t value) {
    ...
    DeviceIoControl(ptedit_fd, PTEDITOR_WRITE_PHYS_VAL, (LPVOID)&info,
                    sizeof(info), (LPVOID)&info, sizeof(info), &returnLength,
                    0);
#endif
}
...
static ptedit_entry_t ptedit_resolve_user_ext(void* address, pid_t pid,
    ...
    if (!ptedit_cast(pmd_entry, ptedit_pmd_t).size) {
        ...
        pt_entry = deref(pfn * ptedit_pfn_multiply +
                         pti * ptedit_entry_size);  // pt[pti];
        resolved.pte = pt_entry;
        resolved.valid |= PTEDIT_VALID_MASK_PTE;
        if (ptedit_cast(pt_entry, ptedit_pte_t).present !=
            PTEDIT_PAGE_PRESENT) {
            return resolved;
        }
#if defined(__i386__) || defined(__x86_64__) || defined(_WIN64)
...
ptedit_fnc void ptedit_print_entry_line(size_t entry, int line) {
    ...
    if (line == 2) {
        printf("|");
        PEDIT_PRINT_B(
            "%2d", (PTEDIT_B(entry, 63) << 4) | (PTEDIT_B(entry, 62) << 3) |
                       (PTEDIT_B(entry, 61) << 2) | (PTEDIT_B(entry, 60) << 1) |
                       PTEDIT_B(entry, 59));
        PEDIT_PRINT_B("%2d",
                      (PTEDIT_B(entry, 58) << 3) | (PTEDIT_B(entry, 57) << 2) |
        ...
        PEDIT_PRINT_B("%2d",
                      (PTEDIT_B(entry, 51) << 3) | (PTEDIT_B(entry, 50) << 2) |
...
#else
    ptedit_fd = CreateFile(PTEDITOR_DEVICE_PATH, GENERIC_ALL, 0, 0,
...
ptedit_fnc void ptedit_use_implementation(int implementation) {
    if (implementation == PTEDIT_IMPL_KERNEL) {
        ...
        ptedit_paging_root = ptedit_get_paging_root(0);
        if (!ptedit_vmem) {
            ptedit_vmem = (unsigned char*)mmap(
                NULL, 32ull << 30ull, PROT_READ | PROT_WRITE,
...
#if defined(LINUX)
    if (ptedit_umem > 0) {
        if (pread(ptedit_umem, buffer, ptedit_pagesize,
                  pfn * ptedit_pagesize) == -1) {
            return;
        }
    } else {
        ptedit_page_t page;
...
#if defined(LINUX)
    if (ptedit_umem > 0) {
        if (pwrite(ptedit_umem, content, ptedit_pagesize,
                   pfn * ptedit_pagesize) == -1) {
            return;
        }
    } else {
...
ptedit_fnc void ptedit_invalidate_tlb(void* address) {
    ...
    DWORD returnLength;
    DeviceIoControl(ptedit_fd, PTEDITOR_FLUSH_TLB, (LPVOID)&vaddr,
                    sizeof(vaddr), (LPVOID)&vaddr, sizeof(vaddr), &returnLength,
                    0);
#endif
}
